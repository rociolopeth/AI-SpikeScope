...
def load_nsx(file_path: str, plot: bool = True) -> Dict[str, Any]:
    """
    ...
    if plot:
        ...
        plt.yticks(np.arange(1, 97) * 5, ['E' + str(it) + ' CH' + str(utah.electrode2channel[str(it)]) for it in np.arange(1, 97)])

    return data_container


def load_triggers(file_path: str) -> Dict:
    """
...
def get_spikes_from_raw(raw_container: Dict[str, np.ndarray], triggers: Optional[List[int]] = None, path: str = '',
                        fs_raw: int = 30000, low: int = 250, std: int = 5, step: int = 12, length: int=50) -> Dict[str, List]:
    """
    ...
    spike_dict = {
        ...
        'Waveforms': [], 'Triggers': [np.array(triggers)] if triggers else [], 'Triggers_active': [True] if triggers else []
    }
    ...
    for electrode in range(1,97):
        ...
        print(' electrode ', electrode, ' applyied threshold_std ', std, ' signal std*threshold_std ', filtered.std()*std, 'threshold ', threshold)
